...
class Crypto extends Model
{
    ...
    protected $fillable = ['lang', 'title', 'slug', 'description', 'sub_category_id', 'assign_featured', 'start_date_time', 'end_date_time','views', 'reservation', 'lat', 'long', 'location', 'country', 'city', 'city_id', 'area','cusine_name', 'view_floor_plan', 'images', 'feature_image', 'stories', 'icon', 'amenity_id', 'landmark_id','whatsapp', 'contact', 'email', 'prices', 'view_menu', 'is_draft', 'is_publisher','video', 'youtube_img', 'map_review', 'map_rating', 'status','meta_tags', 'meta_title', 'meta_description',
        'status_text', 'is_popular', 'popular_types', 'created_by', 'crypto_capacity', 'is_hot', 'is_trending', 'discount_offer'
    ];
    ...
    {
        return Storage::disk('s3')->url(config('app.upload_crypto_path') . $img);
    }
    ...
    {
        return $this->hasMany(CryptoEvent::class);
    }

    public function upcomingEvents()
    {
        return $this->hasMany(CryptoEvent::class)->where('datetime', '>', now());
    }
    ...
    {
        return $this->hasOne(CryptoImage::class)->where('image_type', 'feature_image')->latest();
    }

    /**
     * Get the story images of the crypto.
     */
    public function storyImages()
    {
        return $this->hasMany(CryptoImage::class)->where('image_type', 'stories')->latest();
    }
    ...
    {
        return $this->hasOne(CryptoImage::class)->where('image_type', 'main_image')->latest();
    }

    /**
     * Get the 4  images of the crypto.
     */
    public function mainImages()
    {
        return $this->hasMany(CryptoImage::class)->where('image_type', 'images')->latest()->limit(4);
    }

    public function logoImage()
    {
        return $this->hasOne(CryptoImage::class)->where('image_type', 'logo')->latest();
    }

    public function floorPlanImage()
    {
        return $this->hasOne(CryptoImage::class)->where('image_type', 'floor_plan')->latest();
    }

    public function menuImage()
    {
        return $this->hasOne(CryptoImage::class)->where('image_type', 'menu')->latest();
    }

    public function getStoredImage($img, $img_type = 'feature_image')
    {
        if(Storage::disk('s3')->exists(config('app.upload_crypto_path') . $img_type . '/' . $img)) {
            return Storage::disk('s3')->url(config('app.upload_crypto_path') . $img_type . '/' . $img);
        }

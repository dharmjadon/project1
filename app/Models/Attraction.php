...
class Attraction extends Model
{
    ...
    protected $fillable = ['lang', 'title', 'slug', 'description', 'sub_category_id', 'is_featured',
        'views', 'lat', 'lng', 'location', 'country',
        'city', 'city_id', 'area','images', 'feature_image', 'stories',
        'logo', 'amenties', 'landmarks', 'whatsapp', 'mobile', 'email', 'is_draft',
        'is_publisher', 'video', 'youtube_img', 'map_review', 'map_rating', 'status', 'meta_keywords', 'meta_title',
        'meta_description', 'status_text', 'is_popular', 'popular_types', 'created_by', 
         'is_hot', 'is_trending', 'discount_offer', 'attraction_type'
    ];
    ...
    {
        if(Str::contains($img, 'uploads/attraction')) {
            return Storage::disk('s3')->url($img);
        }
        return Storage::disk('s3')->url(config('app.upload_attraction_path') . $img);
    }
    ...
    {
        return $this->hasMany(AttractionEvent::class);
    }

    public function upcomingEvents()
    {
        return $this->hasMany(AttractionEvent::class)->where('datetime', '>', now());
    }


    public function featureImage()
    {
        return $this->hasOne(AttractionImages::class, 'attraction_id')->where('image_type', 'feature_image')->latest();
    }
    ...
    {
        return $this->hasMany(AttractionImages::class, 'attraction_id')->where('image_type', 'stories')->latest();
    }
    ...
    {
        return $this->hasOne(AttractionImages::class, 'attraction_id')->where('image_type', 'main_image')->latest();
    }
    ...
    {
        return $this->hasMany(AttractionImages::class, 'attraction_id')->where('image_type', 'images')->latest();
    }

    public function logoImage()
    {
        return $this->hasOne(AttractionImages::class, 'attraction_id')->where('image_type', 'logo')->latest();
    }

    public function getStoredImage($img, $img_type = 'feature_image')
    {
        if(Storage::disk('s3')->exists(config('app.upload_attraction_path') . $img_type . '/' . $img)) {
            return Storage::disk('s3')->url(config('app.upload_attraction_path') . $img_type . '/' . $img);
        }

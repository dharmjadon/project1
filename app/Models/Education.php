...
class Education extends Model
{
    ...
    protected $fillable = ['lang', 'title', 'slug', 'description', 'sub_category_id', 'is_featured',
        ...
        'meta_description', 'status_text', 'is_popular', 'popular_types', 'created_by', 'education_capacity',
    ...
    {
        if(Str::contains($img, 'uploads/education')) {
    ...
    {
        return $this->hasOne(EducationImage::class)->where('image_type', 'feature_image')->latest();
    }

    /**
     * Get the story images of the education.
     */
    public function storyImages()
    {
        return $this->hasMany(EducationImage::class)->where('image_type', 'stories')->latest();
    }
    ...
    {
        return $this->hasOne(EducationImage::class)->where('image_type', 'main_image')->latest();
    }

    /**
     * Get the 4  images of the education.
     */
    public function mainImages()
    {
        return $this->hasMany(EducationImage::class)->where('image_type', 'images')->latest();
    }

    public function logoImage()
    {
        return $this->hasOne(EducationImage::class)->where('image_type', 'logo')->latest();
    }

    public function floorPlanImage()
    {
        return $this->hasOne(EducationImage::class)->where('image_type', 'floor_plan')->latest();
    }

    public function menuImage()
    {
        return $this->hasOne(EducationImage::class)->where('image_type', 'menu')->latest();
    }

    public function getStoredImage($img, $img_type = 'feature_image')
    {
        if(Storage::disk('s3')->exists(config('app.upload_education_path') . $img_type . '/' . $img)) {
            return Storage::disk('s3')->url(config('app.upload_education_path') . $img_type . '/' . $img);
        }
